/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   key_handler.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: afruhauf <afruhauf@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/02/11 17:09:06 by afruhauf          #+#    #+#             */
/*   Updated: 2026/02/11 17:09:07 by afruhauf         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

static void	crouch(t_data *data)
{
	data->crouch *= -1;
	if (data->crouch == 1)
	{
		data->c.target_height = 0.3;
		data->move_mult = 0.4;
	}
	else
	{
		data->c.target_height = 0.5;
		data->move_mult = 1.0;
	}
}

static void	dodge(t_data *data)
{
	if (!data->dodge && !data->dodge_timer && data->stamina >= 0.15)
	{
		ma_sound_start(&data->s.dodge);
		data->stamina -= 0.15;
		data->dodge = 1;
		data->dodge_timer = 60;
		data->move_mult += 4.5;
		if (data->crouch != 1 && data->jump == 0)
		{
			data->c.target_height = 0.2;
			data->c.cam_speed_down = 0.05;
		}
	}
}
//if (!ma_sound_is_playing(&data->sound_dodge))

static void	jump(t_data *data)
{
	if (!data->jump && data->stamina >= 0.15)
	{
		ma_sound_start(&data->s.jump);
		data->stamina -= 0.15;
		data->jump = 1;
		data->c.target_height = data->c.cam_height + 0.45;
		data->c.cam_speed_up = 0.03;
		data->c.cam_speed_down = 0.01;
	}
}

void	key_handler(mlx_key_data_t keydata, void *param)
{
	t_data	*data;

	data = (t_data *)param;
	if (keydata.action == MLX_PRESS)
	{
		if (keydata.key == MLX_KEY_ESCAPE)
			clean(data, NULL, 0, -1);
		if (keydata.key == MLX_KEY_C)
			crouch(data);
		if (keydata.key == MLX_KEY_V)
			dodge(data);
		if (keydata.key == MLX_KEY_SPACE)
			jump(data);
	}
}
